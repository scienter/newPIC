    ...
    case 2 :
      ...
      else if(resolX>1 && resolY>1) 
      {
        ...
        for(s=0; s<nSpecies; s++) 
        {
          ...
          resolCalX(density,reField,2,iend,0,jend+3,0,1,resolX,rankX,0);
          resolCalY(reField,reDensity,2,reIend,2,jend,0,1,resolY,rankY,1);
          ...
          for(i=istart; i<reIend; i++)
            for(j=jstart; j<reJend; j++)
            {
              ...
              for(l=0; l<2; l++)
                for(m=0; m<2; m++)
                  sum+=reDensity[i+l][j+m][k];
              sum*=0.25;
              number=(int)(sum*numInCell);
              randTest=sum*numInCell-number;
